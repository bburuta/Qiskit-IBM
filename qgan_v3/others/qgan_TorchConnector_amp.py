...
# %% [markdown]
# ## Implementation: Amplitude encoding with Torch Connector
...
def apply_base_implementation_mode(config):
    if config['encoding_options']['dataset'] != 'base':
        return config

    config['implementation_options']['random_input'] = False
    config['encoding_options']['batch_size'] = 1
    config['encoding_options']['eval_batch_size'] = 1
...
def generate_real_circuits(matrices = None):
    ...
    if (matrices is None) or (matrices is []):
        ...
        # Amplitude encoding
        _, X_amplitudes = images_to_probs(matrices, config['encoding_options']['contrast'],)
...
def create_training_data_file(n_gen_params, n_disc_params, filename):
    ...
    model_d = JoinedDiscriminator(
        ...
        max_workers=config['encoding_options']['batch_size'],
    )
...
def generate_real_input_index(batch_size):
    if batch_size > model_d.num_real_models:
        raise ValueError(f"batch_size={batch_size} is larger than the number of real discriminator models ({model_d.num_real_models}). This is not accepted for safe parallelization. Reduce config['encoding_options']['batch_size'] or create more real discriminator models.")
...
if config['encoding_options']['dataset'] == 'base':
    ...
    X_probs, _ = images_to_probs(X, config['encoding_options']['contrast'],)
...
eval_batch_size = config['encoding_options']['eval_batch_size'] # TODO add in config
