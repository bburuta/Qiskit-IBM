# %% [markdown]
# Version con TorchConnector, parametros guardados junto al state del modelo (pero para visualizar tambien se guardan aparte), gradientes actualizadas sin insercción a mano mediante un torch.nn.Module que une dos instancias de TorchConnector que usan mismos parametros y joint gradient, step te devuelve el loss (no es necesario calcularlo en un paso extra)
...
# File management
def manage_files(data_folder_name = 'data', implementation_name = 'fullyq_torchc3', execution_type_name = 'sim', training_data_file_name = 'training_data', circuits_file_name = 'circuits'):
...
# This model joins both discriminators and calculates backward pass in serial
class JoinedDiscriminator(torch.nn.Module):
    def __init__(self, disc_real_qnn, disc_fake_qnn, initial_weights = None):
        super().__init__()
        self.model_dr = TorchConnector(disc_real_qnn, initial_weights = initial_weights)
        self.model_df = TorchConnector(disc_fake_qnn)
        
        # Force model2 to look at model1's weights
        if 'weight' in self.model_df._parameters:
            self.model_df._parameters.pop('weight')
        if '_weights' in self.model_df._parameters:
            self.model_df._parameters.pop('_weights')
        self.model_df._parameters['weight'] = self.model_dr.weight
        self.model_df._parameters['_weights'] = self.model_dr.weight

    def forward(self, x):
        out1 = self.model_dr(x)
        out2 = self.model_df(x)
        return (out1 + out2) / 2
...
def reset_data(gen_qnn, disc_real_qnn, disc_fake_qnn):
    np.random.seed(train_config['seed'])

    init_gen_params = np.random.uniform(low=-np.pi, high=np.pi, size=(gen_qnn.num_weights,))
    init_disc_params = np.random.uniform(low=-np.pi, high=np.pi, size=(disc_fake_qnn.num_weights,))
    ...
    # --- VERIFICATION ---
    print("\nNumber of parameters: ", len(list(model_d.model_dr.parameters())), len(list(model_d.model_df.parameters()))) # Should be 1
    print(f"Are weights tied? {model_d.model_df.weight is model_d.model_dr.weight}") # Should be True
    print(list(model_d.parameters()))
...
if train_config['reset_data']:
    reset_data(gen_qnn, disc_real_qnn, disc_fake_qnn)
...
try: # In case of interruption
    for epoch in range(current_epoch, train_config['max_iterations']+1):
        ...
        for disc_train_step in range(D_STEPS):
            ...
            gen_loss = f_loss(real_output, torch.ones(1))
            real_loss = f_loss(real_output, torch.ones(1)) # 1-> Real guess (correct) # TODO device=device)
...
#--- Save parameters and optimizer states data ---#
    ...
    torch.save({
        ...
        'disc_params': torch.nn.utils.parameters_to_vector(model_dr.parameters()),
        'model_g_state': model_g.state_dict(),
        'model_dr_state': model_dr.state_dict(),
        'model_df_state': model_df.state_dict(),
        'optimizer_g_state': optimizer_g.state_dict(),
        'optimizer_d_state': optimizer_d.state_dict(),
