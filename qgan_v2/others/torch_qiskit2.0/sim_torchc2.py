# %% [markdown]
# Version con TorchConnector, parametros guardados junto al state del modelo (pero para visualizar tambien se guardan aparte), dos modelos de discriminador para datos reales y generados que comparten sus parametros (cambiando los linkers a mano), step te devuelve el loss (no es necesario calcularlo en un paso extra)
...
#pip install qiskit==2.3 qiskit-machine-learning==0.9.0 'qiskit-machine-learning[sparse]' torch
...
# File management
def manage_files(data_folder_name = 'data', implementation_name = 'fullyq_torchc2', execution_type_name = 'sim', training_data_file_name = 'training_data', circuits_file_name = 'circuits'):
...
def reset_data(gen_qnn, disc_real_qnn, disc_fake_qnn):
    ...
    torch.save({
        ...
        'disc_params': torch.nn.utils.parameters_to_vector(model_dr.parameters()).detach().numpy(),
