...
from qgan_v4.circuits.encoding import images_to_prob
from qgan_v4.datasets.images import get_image_shape
from qgan_v4.datasets.quantum import circuits_to_probs
from qgan_v4.training.batch_torch import generate_random_input, generate_real_input_index
...
def evaluate_gradient(gen_dists, target=None, image_shape=None):
    ...
    h_penalty = -h_diff.clamp(max=0).mean() if h_diff.numel() > 0 else batch.new_tensor(0.0)
    v_penalty = -v_diff.clamp(max=0).mean() if v_diff.numel() > 0 else batch.new_tensor(0.0)
...
def batch_evaluation(eval_method, randomness, eval_g, batch_size, num_random_params, device, dtype, target_probs=None, image_shape=None, eval_weights=None):
    evaluate, compute_targets = get_evaluation_function(eval_method, image_shape=image_shape)

    with torch.no_grad():
        # Get fake samples
        random_input = generate_random_input(batch_size, num_random_params, randomness, device, dtype)
        if eval_weights is None:
            fake_outputs = eval_g(random_input)
        else:
            fake_outputs = eval_g(random_input, eval_weights)
        ...
        if compute_targets:
            ...
            real_indexes = generate_real_input_index(batch_size, target_probs.shape[0], device)
