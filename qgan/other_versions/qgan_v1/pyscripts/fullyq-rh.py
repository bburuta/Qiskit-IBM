#- Fully Quantum GAN for real hardware -#

# Execution example
#python3 fullyq-rh.py --n_qubits 4 --seed 100 --print_progress 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--real_hardware", required=False, type=int, default=0)
...
def generate_training_circuits(session, real_circuit, generator, discriminator):
    ...
    isa_circuits = [pm.run(real_disc_circuit), pm.run(gen_disc_circuit)]
    isa_observables = [H1.apply_layout(isa_circuits[0].layout), H1.apply_layout(isa_circuits[1].layout)]
...
try: # In case of interruption
    with Session(backend=backend) as session:
        #--- Create QNNs in session ---#
        gen_qnn, disc_fake_qnn, disc_real_qnn = generate_training_circuits(session, real_circuit, generator, discriminator)
