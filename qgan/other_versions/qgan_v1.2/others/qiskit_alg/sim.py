...
#pip install qiskit==2.3 qiskit-machine-learning==0.9.0 'qiskit-machine-learning[sparse]' qiskit_algorithms matplotlib pylatexenc ipykernel
...
#from qiskit.quantum_info import relative_entropy TODO prueba a ver
...
#from qiskit_algorithms.optimizers import ADAM TODO
...
# File management
def manage_files(data_folder_name = 'data', implementation_name = 'fullyq', execution_type_name = 'sim', training_data_file_name = 'training_data', circuits_file_name = 'circuits'):
    ...
    training_data_file = data_folder + training_data_file_name + '.pkl'
...
def reset_data(n_gen_params, n_disc_params):
    ...
    optimizer_g = ADAM(max_iter=1, lr=0.01) # TODO
    optimizer_d = ADAM(max_iter=1, lr=0.01)

    # result = optimizer.minimize(fun=objective_function, x0=params, state=opt_state) # TODO save state of optimizers
    
    # # 4. Update variables for next iteration
    # params = result.x
    # opt_state = result.state # CRITICAL: Keeps ADAM's internal memory alive
    ...
    with open(train_config['training_data_file'], 'wb') as file:
...
try:
    with open(train_config['training_data_file'], 'rb') as file:
...
optimizer_g.load_state_dict(params['optimizer_g_state']) # TODO OptimizerState?
...
real_distribution = Statevector(real_circuit).probabilities() # Retrieve real data probability distribution 
...
try: # In case of interruption
    for epoch in range(current_epoch, train_config['max_iterations']+1):
        ...
        gen_distribution = Statevector(generator_circuit.assign_parameters(gen_params_np)).probabilities() # Retrieve probability distribution of generator with current parameters.

        # Performance measurement function: uses Kullback Leibler Divergence to measures the distance between two distributions
        current_kl = f_evaluate.evaluate(gen_distribution, real_distribution)
