...
from encoding import create_real_circuits, create_randomizer_circuit
...
# Generate circuits SHA
def generate_circuits_sha(circuits):
    circuits_data = BytesIO()
    qpy.dump(circuits, circuits_data)

    return hashlib.sha256(circuits_data.getvalue()).hexdigest()
