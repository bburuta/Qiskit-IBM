...
def generate_train_qnns(real_disc_circuits_transpiled, gen_disc_circuit_transpiled, obs_real_disc, obs_gen_disc, gradient, estimator):
    ...
    n_disc_params = sum(param.name.startswith("θ_d") for param in gen_disc_params)
    n_gen_params = sum(param.name.startswith("θ_g") for param in gen_disc_params)
    ...
    gen_qnn = EstimatorQNN(
        ...
        input_params=gen_disc_params[:n_disc_params] + gen_disc_params[n_disc_params + n_gen_params:], # fixed parameters (discriminator + random parameters)
        weight_params=gen_disc_params[n_disc_params:n_disc_params + n_gen_params], # parameters to update (generator parameters)
        estimator=estimator,
    ...
    disc_fake_qnn = EstimatorQNN(
        circuit=gen_disc_circuit_transpiled,
        input_params=gen_disc_params[n_disc_params:], # fixed parameters (generator + random parameters)
        weight_params=gen_disc_params[:n_disc_params], # parameters to update (discriminator parameters)
        estimator=estimator,
        observables=obs_gen_disc,
        gradient=gradient,
        default_precision=training_precision,
    ...
    for real_disc_circuit_transpiled, obs in zip(real_disc_circuits_transpiled, obs_real_disc):
        ...
        disc_real_qnn = EstimatorQNN(
            ...
            input_params=real_disc_params[n_disc_params:], # fixed parameters (real data parameters)
            weight_params=real_disc_params[:n_disc_params], # parameters to update (discriminator parameters)
            estimator=estimator,
            observables=obs,
            gradient=gradient,
            default_precision=training_precision,
...
def generate_eval_qnn_ang(gen_eval_circuit_transpiled, obs_gen_eval, gradient, eval_estimator):
    ...
    n_gen_params = sum(param.name.startswith("θ_g") for param in gen_eval_params)
    eval_precision = eval_estimator.options.default_precision

    gen_eval_qnn = EstimatorQNN(
        circuit=gen_eval_circuit_transpiled,
        input_params=gen_eval_params[n_gen_params:], # fixed parameters (random parameters)
        weight_params=gen_eval_params[:n_gen_params], # parameters to update (generator parameters)
