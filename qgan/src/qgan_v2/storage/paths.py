...
def get_backends_path():
    return get_qgan_root() / "backends"



#- Run storage paths -#

# Resolve absolute, repository-relative, or qgan-relative run data path
def resolve_data_path(data_path):
    ...
    if path.parts and path.parts[0] == qgan_root.name:
        return get_repository_root() / path

    return qgan_root / path
